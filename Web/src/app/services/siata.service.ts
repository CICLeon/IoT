...
import { IDataBD, IResource, ISIATAPM25 } from '../interfaces/siata';
...
export class SiataService {
    ...
    getData() {
        ...
        return this.http.get<IDataBD[]>(url, { observe: 'response', headers: this.headers })
            .pipe(
                map(resp => resp.body),
                catchError((err) => of(typeof (err.error) === 'string' ? err.error : 'Coctacte al proveedor'))
            )
    }

    getResource() {
        const url = `${this.baseUrl}/${this.controller}/Resource`;
        return this.http.get<IResource[]>(url, { observe: 'response', headers: this.headers })
            .pipe(
                map(resp => resp.body),
                catchError((err) => of(typeof (err.error) === 'string' ? err.error : 'Coctacte al proveedor'))
            )
    ...
    saveRecourse(model: IResource) {
        const url = `${this.baseUrl}/${this.controller}/Resource`;
        const body: IResource = model;
